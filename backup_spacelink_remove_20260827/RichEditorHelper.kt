...
object RichEditorHelper {
    ...
    fun extractReferences(markdown: String): List<ContentReference> {
        ...
        SPACE_FILE_PATTERN.findAll(markdown).forEach { match ->
            val id = match.groupValues[1].toLongOrNull()
            if (id != null) {
                references.add(ContentReference(refType = RefType.SPACE_FILE, targetId = id))
            }
        }
    ...
    fun createSpaceFileShortcode(
        ...
        val thumbnail = thumbnailUrl ?: ""
        return "[spacefile][$id][$typeStr][$name][$thumbnail]"
    }
    ...
    /**
     * 从 Markdown 中提取空间文件短代码的信息
     */
    fun extractSpaceFileInfo(markdown: String): Map<Long, SpaceFileInfo> {
        val result = mutableMapOf<Long, SpaceFileInfo>()
        SPACE_FILE_PATTERN.findAll(markdown).forEach { match ->
            val id = match.groupValues[1].toLongOrNull()
            val type = match.groupValues[2]
            val name = match.groupValues[3]
            val thumbnailUrl = match.groupValues[4]
            if (id != null) {
                result[id] = SpaceFileInfo(type, name, thumbnailUrl)
            }
        }
        return result
    }
    ...
    fun updateSpaceFileShortcode(
        ...
        val newShortcode = createSpaceFileShortcode(id, type, name, thumbnailUrl)
    ...
    /**
     * 从 Markdown 中移除所有短代码（[image]、[video]、[voice]、[spacelink]、[spacefile] 等）
     * 以及 HTML 标签，仅保留纯文字内容，用于摘要显示
     */
    fun stripShortcodesForSummary(markdown: String): String {
        var result = markdown
            .replace(Regex("<br\\s*/?>"), " ")
            .replace(Regex("<[^>]+>"), " ")
        val shortcodeTypes = listOf("image", "video", "voice", "spacelink", "spacefile")
        shortcodeTypes.forEach { type ->
            val regexStr = "\\[$type](?:\\[[^\\]]*])+"
            result = result.replace(Regex(regexStr), " ")
        }
        result = result
            .replace(Regex("\\s+"), " ")
            .trim()
